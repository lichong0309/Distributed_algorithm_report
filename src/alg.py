...
class FLlearningNonIID_1(object):
    def __init__(self):
        ...
        self.data_per_client =[ [ 271, 0,  157,  563, 1002,    4,  362,  801, 1170,  670],
 [ 470,   87,  1364,  482,  464,  467,  635,  131,  481,  419],
 [ 525,  498,  188,  552,  896,  306, 1051,  116,  414,  454],
 [ 501,  163, 1445,  285,  737,  192,  379,  169,  473,  656],
 [ 712,  362,  206,  889,  223,  622,  528,  832,  254,  372],
 [ 569,  225,  364,  327,  542,  179,  956,  740,  195,  903],
 [ 875,  391,  322,  989,  853,  181,  800,  108,  188,  293],
 [ 271,  357,  414,  420,  497,  523,  143,  995,  663,  717],
 [ 271,  847,  362,  635,  603,  205,  355,  108,  397, 1217],
 [ 407,  274,  197,  628,  857,  702,  258,  108,   0, 1569]]
    ...
    def split_image_data(self,data, labels, n_clients=100, classes_per_client=10, shuffle=True, verbose=True):
        ...
        for i in range(n_clients):
            client_idcs = []
            for j in range(10):
                if self.data_per_client[i][j] == 0:
                    pass
                else:
                    take = self.data_per_client[i][j]
                    client_idcs += data_idcs[j][:take]
                    data_idcs[j] = data_idcs[j][take:]
                    print("test_1",len(data_idcs[j]))
            print("test:",len(client_idcs))
            clients_split += [(data[client_idcs], labels[client_idcs])]
        # print("clients_split:",clients_split)
    ...
    def get_data_loaders(self,nclients,batch_size,classes_pc=10 ,verbose=True ):
        ...
        client_loaders = [torch.utils.data.DataLoader(CustomImageDataset(x, y, transforms_train), 
                                                                        batch_size=batch_size, shuffle=False) for x, y in split_tmp]
    ...
    def client_update(self, client_model, optimizer, train_loader, epoch, model):
        ...
        for e in range(epoch):
            for batch_idx, (data, target) in enumerate(train_loader):
                ...
                loss_q = F.nll_loss(output, target)
                loss_q.backward()
                optimizer.step()
        return loss_q.item()
