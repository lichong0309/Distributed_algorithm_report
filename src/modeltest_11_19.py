...
class FLNonIIDModel(object):
    def __init__(self):
        ...
        self.num_selected = self.num_clients            # 进行训练的主机的个数,设置进行训练的主机个数和环境中的主机个数相等
        self.num_rounds = 100                           # 主机和服务器之间进行通信的轮数    
...
###### Loading the data using the above function ######
    def load_data(self):
        self.train_loader, self.test_loader = self.get_data_loaders(classes_pc=self.classes_pc, nclients= self.num_clients,
    ...
    def clustertrain(self, neural_network_name, client_idx):
        print("test_1")
        ############################################
        ...
        #### global model ##########
        global_model =  VGG(neural_network_name).cuda()

        ############# client models ###############################
        client_models = [ VGG(neural_network_name).cuda() for _ in range(len(client_idx))]
        for model in client_models:
        ...
        for r in range(self.num_rounds):    #Communication round
            ...
            client_lens = [len(self.train_loader[idx]) for idx in client_idx]             # 得到selected的client对应的数据. len(client_lens) 为数据量的大小

            # client update
            loss = 0
            for i in tqdm(range(len(client_idx))):
                self.client_syn(client_models[i], global_model)
                loss += self.client_update(client_models[i], opt[i], self.train_loader[client_idx[i]], self.epochs,model)
    ...
    def main(self):
        self.load_data()
        client_idx_0 = [0, 1, 2, 3, 4]              # 类别0的client的编号
        client_idx_1 = [5, 6, 7, 8, 9]              # 类别1的client的编号
        neural_network_name_0 =  'VGG11'                # 类别0的NN类型，VGG11 
        neural_network_name_1 =  'VGG19'                # 类别1的NN类型，VGG19
        acc_test_0, losses_test_0 = self.clustertrain(neural_network_name_0, client_idx_0)   # 类别0的acc和loss
        acc_test_1, losses_test_1 = self.clustertrain(neural_network_name_1, client_idx_1)   # 类别1的acc和loss
        # 计算每个类别的数据量
        data_all_0 , data_all_1 = 0, 0                  # 初始化数据量 
        for i in client_idx_0:
            data_all_0  = data_all_0 + self.dataallocation[i]
        for j in client_idx_1:
            data_all_1 = data_all_1 + self.dataallocation[j]
        data_all = data_all_0 + data_all_1              # 参与训练的总的数据量
        acc_test, losses_test = [], []                         # 创建acc和losses空列表，存放self.num_rounds次产生的acc和losses
        acc_test_temp, losses_test_temp = 0, 0                      # 初始化acc和losses的暂存变量
        for m in range(self.num_rounds):
            acc_test_temp = acc_test_0[m] * (data_all_0 / data_all) + acc_test_1[m] * (data_all_1 / data_all)                # 根据数据量计算总的acc
            losses_test_temp = losses_test_0[m] * (data_all_0 / data_all) + losses_test_1[m] * (data_all_1 /data_all)    # 根据数据量计算总的losses
            acc_test.append(acc_test_temp)
            losses_test.append(losses_test_temp)
        return acc_test, losses_test
